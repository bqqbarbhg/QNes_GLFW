...
void drawChar2D(char c, vec2 p, vec2 d, vec2 r)
{
	if(font[c])
	{
		for(int x = 0; x < 5; x++)
		{
			for(int y = 0; y < 5; y++)
			{
				if(font[c] & (1 << (x * 5 + y)))
				{
					float xf = (float)x;
					float yf = (float)y;
					glVertex2fv(value_ptr(p + xf * r + yf * d));
					glVertex2fv(value_ptr(p + (xf + 1.0f) * r + yf * d));
					glVertex2fv(value_ptr(p + (xf + 1.0f) * r + (yf + 1.0f) * d));
					glVertex2fv(value_ptr(p + xf * r + (yf + 1.0f) * d));
				}
			}
		}
	}
...
void drawString2D(char* s, vec2 p, vec2 d, vec2 r, float c)
{
	glBegin(GL_QUADS);
